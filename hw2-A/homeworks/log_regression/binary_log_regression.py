...
class BinaryLogReg:
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        log_likelihood = np.mean(np.log(1 + np.exp(-y * (self.bias + np.dot(X, self.weight)))))
        regularization = self._lambda * np.sum(self.weight ** 2)
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        return -np.dot(X.T, y * (1 - self.mu(X, y))) / X.shape[0] + 2 * self._lambda * self.weight
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        predict = self.mu(X, np.ones(X.shape[0]))
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for i in range(num_batches):
                ...
                X_batch = shuffleX[batch_start:batch_end]
                y_batch = shuffleY[batch_start:batch_end]
...
if __name__ == "__main__":
    ...
    history = model.train(x_train, y_train, x_test, y_test, batch_size=len(x_train))
    ...
    history = model.train(x_train, y_train, x_test, y_test, batch_size=1)
    ...
    history = model.train(x_train, y_train, x_test, y_test, batch_size=100)
