...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    AxT = vanilla_matmul(vanilla_transpose(A), x)
    By = vanilla_matmul(vanilla_transpose(B), y)
    result: Vector = np.array(Ax) + np.array(AxT) + np.array(By)
    end = time.time()
    
    print("\n vanilla runtime: ", end - start)
    return result
...
def numpy_solution(
    ...
    start = time.time()
    Ax = np.matmul(A, x)
    AxT = np.matmul(np.transpose(A), x)
    By = np.matmul(np.transpose(B), y)
    result: Vector = Ax + AxT + By
    end = time.time()

    print("\n numpy runtime: ", end - start)
    return result
